...
deliberate miss. It has six headings, and five carry `text-wrap: balance`:
...
That heading was the single word `Reminders` until the round of 2026-08-17. The
...
finding of its own. An id does not count as reported when it appears only in
another finding's kill list or body, even if the run says it fires.
...
declined list. A grep of the artifact confirms that neither id is there. This is
the same suppression with nothing written down at all. None of the six reads
...
- **C4's second door was a judgment, not a rule.** The full run on
  `clean-dashboard` declined C4 with this reasoning: "One heading without the property against
  four with it reads as a single oversight in a codebase that otherwise applies
  the treatment consistently, not as a pattern of nobody checking." The
...
pair is asymmetric, the one figure that moves carries `tabular-nums`, and all
eight borders declare both themes. The Signal never matched, so a door that
...
- **C1 on the chip inside the panel.** `rounded-chip` (999px,
  ...
  `components/table.tsx:48`). Its `px-5 py-2` padding is 20px and 8px,
  ...
  by an outer wrap." C1 does not contain that exemption for pills.
...
where the tell should carry a rule. Two remain live on the fixture whose job is
to prove the exemptions hold. C1 and C5 are not repaired
...
run it blind again. The demanded verdict stays the same. Only the mechanism
that produces it returns, so the fixture is not being edited to suit a tell.
...
because each count is degenerate. That is exactly why the corpus will not catch
it. Inherited rather than introduced by this round, and
...
color in the tree. A1 still fires because `theme.extend` is empty. Run 1 reached
it as ROOT from exactly that evidence, so the
...
> exemption ("the list is genuinely immutable, never reordered or
...
reported clause three as false: "so every color in the tree is a framework
default called by its number." It found three counterexamples: a hand-typed hex in
...
untouched `components/ui/` is **dangling** and resolves to nothing. "The tree
...
The three runs supplied ten rules. Each one marks a place where reaching a
verdict required something the tell does not say. Three are structural.
...
evidence that someone looked." It then located the hole: "SKILL.md has exactly
...
`clean-dashboard` fail the second test, but the first cannot see them:
...
same colour for the other theme. Two components render one semantic state, but
only one was opened in dark mode. That is almost C10's Principle word for word.
...
`anti-slop-build/SKILL.md`, `deriving.md` and its brief. It could not read the
fixtures, the auditor or its catalog. A builder that reads
...
`deriving.md` carries no entry for typography treatment at all, so nothing told
the builder that headings and short text blocks take a wrap property. The same
builder had already reported, independently, that spacing, measure and type
...
been stripped from this tree, my tell-by-tell result would be identical." It
named two pieces of prose that tried and failed to move a verdict: the Open Graph
...
which stops framework utilities from compiling. That satisfies A1's first
conjunct on a technicality. It changed no verdict here, because the other conjuncts fail
...
800-weight display face. A2 declined because the project replaces both the
colour scale and the background-image scale, so the utilities named by the
Signal do not exist and `from-purple-*` cannot be typed. It also declined because the gradient
...
as a limit of the product." The code still disables the control at 24 without
explaining why, which is the alternative the sentence rejects. Others are plain
miscounts stated as reasoning: "the two Actions" where three render, "the same
six classes" where four are emitted, and the extraction rule "Two callsites is
the threshold." That rule appears twice and is broken by a wordmark typed
...
family. The builder invented a spacing rule that worked and admitted it: "that
is exactly the kind of decision the skill exists to stop me improvising." It was
...
shape available, so the builder attached a contrast ratio to give it one. The
builder admitted that this "is honest but is not why I picked teal."
...
step, "run `anti-slop` against what you built," does not say what to do without
the auditor. This run had no auditor available.
...
this round the run supplied a bounded one and measured the glyph's area-weighted
centroid at 1.0px off centre. It declined but flagged the call as overturnable.
...
nestings. As the run put it, "the theme comment describes a habit the components
mostly do not keep." And `components/table.tsx:22` calls 7px "the theme's inset" when no
...
move this run's verdict, which came from the count. The run called it "the
...
**A visible defect no tell carries.** The closed filter panel uses `opacity-0`
without `hidden`, so its layout box permanently reserves about 52px between
...
Neither could run `anti-slop` because it sits inside a directory the boundary
forbids. Neither is scored against the catalog here. What they measured is
...
the 'always', which manufactures exactly what the same section forbids," and
...
**Judgment was the most-used addition.** One build used it five times for type
family, gradient, seed radius, selection colour and wordmark. Without it, the
build would have "bolted a contrast ratio onto the Syne choice, which is
...
- **`Judgment` is not listed on every `Record as` line it applies to.**
  Elevation and Both-themes are two examples, so it reads as available in some
